...
def get_args(addl_args=None):
    ...
    parser.add_argument(
        "--nwords",
        help="The number of words to grab from either side total (nwords/2 before and nwords/2 after) of the event mention to generate the training example",
        type=int,
        default=125,
    )
    parser.add_argument(
        "--section",
        help="Designate which section of the label to parse. Options are AR (adverse reactions), BW (boxed warnings), or WP (warnings and precautions)",
        type=str,
        default="AR",
    )
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Proportion of nwords that should come from before the event term. Default is 0.5 (50%%).",
    )

    if addl_args is not None and isinstance(addl_args, list):
    ...
    if args.nwords == 3 and args.method != 0:
        raise Exception(
            "ERROR: method must be set to 0 if nwords is set to 3, it is a special case where only the llt is included as the example."
        )
    ...
    if args.method == 0:
        ...
        raise Exception(
            f"Expected method argument to be an integer value (0-11). Got {args.method}"
        )

    if args.prop_before < 0 or args.prop_before > 1:
        raise Exception(
            f"ERROR: Unexpected value ({args.prop_before}) provided for --prop-before. Needs to be a float between 0 and 1, inclusive."
        )
...
def load_meddra():
    ...
    for row in reader:
        ...
        llts[data["llt_concept_code"]] = (
            data["llt_concept_name"].lower(),
            data["pt_concept_name"],
            data["pt_concept_code"],
        )
...
def load_deepcadrme():
    ...
    for (
        ...
        deepcadrme[drugname].append(
            (term, start, length, match_method, meddra_id, meddra_string)
        )
...
def get_annotations(drug, section_display_name):
    final_ref_fn = (
        "./data/200_manual_annotations_csv/FinalReferenceStandard200Labels.csv"
    )
...
def generate_example(
    ...
    if not source is None:
        if source == "exact":
            ...
            raise Exception(
                f"ERROR: Encountered unexpected source value: {source}. Expected either 'exact' or 'deepcadrme'."
            )
    ...
    if len(example_string.split()) > (nwords + length):
        raise Exception(
            f"ERROR: Example string is too long for term={term}, was length {len(example_string.split())}, expected less than {nwords}"
        )
...
def generate_examples(
    ...
    for i in range(len(parts) - 1):
        ...
        if len(example_string.split()) > (nwords + size_of_llt):
            raise Exception(
                f"ERROR: Example string is too long for llt={llt}, was length {len(example_string.split())}, expected less than {nwords}"
            )
...
def main():
    ...
    for section in sections:
        ...
        train_drugs = set(
            [
                fn.split("_")[0]
                for fn in os.listdir("./data/200_training_set")
                if fn.find(suffix) != -1
            ]
        )
        test_drugs = set(
            [
                fn.split("_")[0]
                for fn in os.listdir("./data/200_test_set")
                if fn.find(suffix) != -1
            ]
        )
        ...
        for drug in all_drugs:
            ...
            pts_annotated, llts_annotated, string_annotated = get_annotations(
                drug, section_display_name
            )
            ...
            print(
                f"\tIntersection of terms with local meddra map: {len(string_annotated & set_meddra_terms)}"
            )
            ...
            if os.path.exists(ar_file_path):
                ...
                root = tree.getroot()
                xml_sections = root.findall(
                    "./Text/Section[@name='%s']" % section_deepcadrme_names[section]
                )
                if len(xml_sections) != 1:
                    raise Exception(
                        "ERROR: Unexpected number of sections named %s. Expected 1 found %s."
                        % (section_deepcadrme_names[section], len(xml_sections))
                    )
            ...
            if os.path.exists(ar_file_path):
                ...
                if os.path.exists(ar_file_path):
                    ...
                    raise Exception(
                        "Couldn't find adverse reactions file in either the training or testing set."
                    )
            ar_text = " ".join(ar_fh.read().split()).lower()

            print(
                f"\tNumber of words in {section_display_name} text: {len(ar_text.split())}"
            )
            ...
            for meddra_id, (
                ...
                for i in range(len(li) - 1):
                    ...
                    start_pos = sum([len(li[j]) for j in range(i + 1)]) + i * len(
                        llt_meddra_term
                    )
                    if (
                        not llt_meddra_term
                        == ar_text[start_pos : (start_pos + len(llt_meddra_term))]
                    ):
                        raise Exception(
                            f" llt_meddra_term: '{llt_meddra_term}', term_in_text: '{ar_text[start_pos : (start_pos + len(llt_meddra_term))]}'"
                        )
                    found_terms.append(
                        (
                            llt_meddra_term,
            ...
            print(
                f"\tFound {len(found_terms)} terms using exact string matches. Took {time.time() - start_time}s."
            )
            ...
            print(
                f"\tFound {len(found_terms)} terms using both exact and DeepCADRME. Took {time.time() - start_time}s."
            )
            ...
            for (
                ...
                if (
                    meddra_id in llts_annotated
                    or meddra_id in pts_annotated
                    or pt_meddra_id in pts_annotated
                ):
                ...
                example_string = generate_example(
                    label_text,
                    found_term,
                    start_pos,
                    length,
                ...
                if drug in train_drugs:
                    ...
                    raise Exception(
                        f"ERROR: Drug {drug} not found in either train_drugs or test_drugs. This shouldn't happen."
                    )
            ...
            print(
                f"\tNumber of positive training examples (from deepcadrme): {num_pos - num_pos_exact}"
            )
            print(
                f"\tNumber of negative training examples (from deepcadrme): {num_neg - num_neg_exact}"
            )
