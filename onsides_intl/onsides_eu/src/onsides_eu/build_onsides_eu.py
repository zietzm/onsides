...
def build(
    ...
    tabular_df["exact_match_list"] = tabular_df["exact_match_list"].apply(
        lambda x: ast.literal_eval(x)
    )
    tabular_df["matched_codes"] = tabular_df["matched_codes"].apply(
        lambda x: ast.literal_eval(x)
    )

    tab_df = tabular_df.explode(["exact_match_list", "matched_codes"])[
        ["drug", "exact_match_list", "matched_codes"]
    ]
    assert isinstance(tab_df, pd.DataFrame)
    tab_df.columns = ["drug", "pt_meddra_term", "pt_meddra_id"]
    txt_df = free_text_df[["label_id", "pt_meddra_term", "pt_meddra_id"]]
    assert isinstance(txt_df, pd.DataFrame)
    ...
    # read in external athena data.
    # TODO : incorporate this step into map_drugs_to_rxnorm.py instead of here.
    rxnorm = pd.read_csv(
    ...
    assert isinstance(rxnorm, pd.DataFrame)
    ...
    ingr_df["rxnorm_code"] = ingr_df["rxnorm_code"].apply(
        lambda x: [str(i) for i in x] if x != "" else ""
    )
    ingr_df["omop_concept_id"] = ingr_df["rxnorm_code"].apply(
        lambda x: [rxnorm_concept_rxnorm[i] for i in x if i in rxnorm_concept_rxnorm]
        if x != ""
        else ""
    )
    ingr_final = ingr_df[
    ...
    ade_all_labels = ade_all_labels.merge(
        ingr_df, left_on="drug", right_on="Medicine name", how="left"
    )
    ade_all_labels = ade_all_labels[
        [
            "Product number",
            "Medicine name",
            "International non-proprietary name (INN) / common name",
    ...
    assert isinstance(ade_all_labels_active, pd.DataFrame)
    ade_all_labels_active.to_csv(
        final_folder / "adverse_events_active_labels.csv", index=False
    )

    # finally, generate the main dataframe
    assert isinstance(ade_all_labels, pd.DataFrame)
    ade_all_labels["ingredient_codes"] = ade_all_labels.ingredient_codes.apply(
        lambda x: str(x).replace("'", "").replace("[", "").replace("]", "")
    )
    ade_all_labels["pt_meddra_id"] = ade_all_labels["pt_meddra_id"].apply(
        lambda x: int(float(x)) if str(x) != "nan" else ""
    )
    uniq_ingr = ade_all_labels[
        ["drug_name", "ingredient_names", "ingredient_codes", "num_ingredients"]
    ].drop_duplicates()
    uniq_ingr = (
        uniq_ingr.groupby("ingredient_codes")["drug_name"].apply(list).reset_index()
    )
    ...
    ade_count = (
        ade_all_labels[["ingredient_codes", "pt_meddra_id"]]
        .groupby("ingredient_codes")["pt_meddra_id"]
        .apply(list)
        .reset_index()
    )
    ade_count["pt_meddra_list"] = ade_count["pt_meddra_id"].apply(
    ...
    ade_count["num_events"] = ade_count.apply(
        lambda x: x["pt_meddra_list"][x["pt_meddra_id"]], axis=1
    )
    ade_all_labels = uniq_ingr.merge(
        ade_count[["ingredient_codes", "pt_meddra_id", "num_events"]],
        on="ingredient_codes",
        how="left",
    )
    ade_all_labels["pct_labels"] = ade_all_labels.apply(
        lambda x: x["num_events"] / x["num_labels"], axis=1
    )
    ade_all_labels = ade_all_labels.merge(uniq_event, on="pt_meddra_id", how="left")

    ades = uniq_ingr.merge(
        ade_count[["ingredient_codes", "pt_meddra_id", "num_events"]],
        on="ingredient_codes",
        how="left",
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        required=True,
        help="Path to the where the final output should be written.",
    )
